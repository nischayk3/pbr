...
class AuditTrials extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      productListPkg: [],
      questionListPkg: [],
      orderSort: { 'old_value': 0, 'activity': 0, 'user_id': 0, 'entry_date': 0, 'table_name': 0 },
      sortState: 'DESC',
      tableData: [],
      userList: [],
      filterIng: [],
  ...
  getExcelFile = (value) => {
    ...
    let startDate = this.state.selectedDate.length > 0 ? this.state.selectedDate[0] : "2021-09-01"
    let endDate = this.state.selectedDate.length > 0 ? this.state.selectedDate[1] : today.toISOString().slice(0, 10)
  ...
  auditHighlight = () => {
    ...
    let req = {
      ...
      startdate: this.state.selectedDate.length > 0 && this.state.selectedDate[0] ? this.state.selectedDate[0] : "2021-09-01",
      enddate: this.state.selectedDate.length > 0 && this.state.selectedDate[1] ? this.state.selectedDate[1] : today.toISOString().slice(0, 10),
    ...
    auditDataChange(req).then((res) => {
      ...
      res.data.forEach((item, key) => {
        ...
        if (val11 === item.new_value) {
          antdObj["new_value"] = <p style={{ background: "yellow" }}>{item.new_value}</p>
        }
        else if (item.activity === "U" && item.delta.length > 0) {
          let val = ""
          item.delta.forEach(item1 => {
            if (val.length > 0) {
              let val1 = val.replace(item1, `<span style='background-color:yellow'>${item1}</span>`)
              val = val1
            } else {
              let val1 = item.new_value.replace(item1, `<span style='background-color:yellow'>${item1}</span>`)
              val = val1
            }
          })
          antdObj["new_value"] = <p dangerouslySetInnerHTML={{ __html: val }}></p>
        }
        else {
  ...
  render() {
    ...
    const userMenu = (
      <Menu>
        <Menu.Item key="1" onClick={() => this.getExcelFile('excel')}>Excel</Menu.Item>
        <Menu.Divider />
        <Menu.Item key="2" onClick={() => this.getExcelFile('csv')}>CSV</Menu.Item>
      </Menu>
    );
    return (
      <div>
        <div className="divFilterDrop">
          <div className="filter-header" style={{ height: '150px' }}>
            ...
            <div className="divFilter-second">
              <Button
                style={{ backgroundColor: '#495fc3', color: "#ffffff", width: "100px" }} type="primary"
                onClick={() => {
                  this.handleFilter();
                }}
              >
          ...
          <div className="custom-table-card" style={{ margin: "10px 0" }}>
            <div className="table-header">
              ...
              <div style={{ marginTop: '10px', float: 'right', marginRight: '10px' }}>
                <Dropdown style={{ color: "#ffffff" }} overlay={userMenu}>
                  <Button style={{ backgroundColor: '#495fc3', color: "#ffffff" }} type="primary">
                    Export
                  </Button>
                </Dropdown>
              </div>
