...
import {
	DeleteOutlined, LeftOutlined, MonitorOutlined, PlusOutlined, RightOutlined
...
function PaperBatchRecordsTemplate() {
	...
	useEffect(() => {
		...
		const getIdTemplateData = async () => {
			...
			if (params?.temp_disp_id || sessionStorage.getItem("test_enabled") == !null) {
	...
	/* istanbul ignore next */
	useEffect(() => {
		if ((templateInfo?.length > 0 && imageWidth !== 0 && imageHeight !== 0) || sessionStorage.getItem("test_enabled") == !null) {
	...
	useEffect(() => {
		dispatch(showLoader());
		if ((imageWidth !== 0 && imageHeight !== 0) || sessionStorage.getItem("test_enabled") == !null) {
			for (let i = 0; i < 2; i++) {
				setTimeout(() => {
	...
	const savePbrTemplateDataInfo = async () => {
		...
		try {/* istanbul ignore next */
			...
			} else if (sessionStorage.getItem("test_enabled") != null) {
				let newReq = {
					pbrTemplateName: params.tempalteName,
					custKey: '1000',
					pbrTemplateVersion: 1,
	...
	const findTemplate = async () => {
		...
		if (sessionStorage.getItem("test_enabled") == null) {/* istanbul ignore next */
			var obj = {
				filename: params?.file,
