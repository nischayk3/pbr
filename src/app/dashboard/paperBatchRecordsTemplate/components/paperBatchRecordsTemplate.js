...
function PaperBatchRecordsTemplate() {
	...
	const parameterAddingHandler = a => {
		...
		if (activeNumber !== 0) {
			if (
				formValues[activeNumber - 1]?.name === '' ||
				formValues[activeNumber - 1]?.name === undefined ||
				formValues[activeNumber - 1]?.method === '' ||
				formValues[activeNumber - 1]?.method === undefined
			) {
				setCallAdd(true);
				openNotification();
	...
	const onChangeChart = (e, field, key, value) => {
		...
		if (field === 'anchorValue') {
			...
			arr[key] = {
				...arr[key],
				values: { ...arr[key]?.values, anchorValue: e.target.value },
			};
			setFormValues(arr);
			...
			arr[key] = {
				...arr[key],
				values: { ...arr[key]?.values, anchorId: e.target.value },
			};
			setFormValues(arr);
			...
			arr[key] = {
				...arr[key],
				unitValues: { ...arr[key]?.unitValues, unitAnchor: e.target.value },
			};
			setFormValues(arr);
			let obj2 = { ...parameterValue };
			obj2[`param${key + 1}`] = {
			...
			arr[key] = {
				...arr[key],
				unitValues: { ...arr[key]?.unitValues, unitId: e.target.value },
			};
			setFormValues(arr);
			let obj2 = { ...parameterValue };
			obj2[`param${key + 1}`] = {
			...
			arr[key] = {
				...arr[key],
				timeValues: { ...arr[key]?.timeValues, timeAnchor: e.target.value },
			};
			setFormValues(arr);
			let obj2 = { ...parameterValue };
			obj2[`param${key + 1}`] = {
			...
			arr[key] = {
				...arr[key],
				timeValues: { ...arr[key]?.timeValues, timeId: e.target.value },
			};
			setFormValues(arr);
			let obj2 = { ...parameterValue };
			obj2[`param${key + 1}`] = {
			...
			arr[key] = {
				...arr[key],
				dateValues: { ...arr[key]?.dateValues, dateAnchor: e.target.value },
			};
			setFormValues(arr);
			let obj2 = { ...parameterValue };
			obj2[`param${key + 1}`] = {
			...
			arr[key] = {
				...arr[key],
				dateValues: { ...arr[key]?.dateValues, dateId: e.target.value },
			};
			setFormValues(arr);
			let obj2 = { ...parameterValue };
			obj2[`param${key + 1}`] = {
	...
	const getImage = async () => {
		...
		let response = await fetch(
			`https://bms-cpvdev.mareana.com/pbr/udh/get_file_page_image?filename=${
				params?.file?.split('_')[0]
			}.pdf&pageId=1`,
			requestOptions
		)
	...
	const clicked = area => {
		...
		if (DraggerActiveMultiple.value) {
			...
			arr[activeKey] = {
				...arr[activeKey],
				values: {
					...arr[activeKey]?.values,
					anchorValue: area.areaValue,
					snippetID: area.snippetID,
					anchorCoords: area.coords,
				},
			};
			setParameterValue(obj1);
			setFormValues(arr);
			...
			arr[activeKey] = {
				...
				values: {
					...arr[activeKey]?.values,
					anchorId: area.areaValue,
					valueSnippetID: area.snippetID,
					valueCoords: area.coords,
				},
			};
			setFormValues(arr);
			setParameterValue(obj1);
		...
		if (formValues.length > 0) {
			try {
				...
				formValues.forEach(ele => {
					...
					if (ele.values) {
						obj['color'] = 'blue';
						obj['param_key_height'] =
							(ele?.values?.anchorCoords[3] - ele?.values?.anchorCoords[1]) /
							1123;
						...
						obj['param_key_width'] =
							ele?.values?.anchorCoords[2] - ele?.values?.anchorCoords[0];
						obj['param_page'] = 1;
						obj['param_snippet_id'] = ele?.values?.snippetID;
						obj['param_value_height'] =
							(ele?.values?.valueCoords[3] - ele?.values?.valueCoords[1]) /
							1123;
						...
						obj['param_value_width'] =
							(ele?.values?.valueCoords[2] - ele?.values?.valueCoords[0]) / 868;
					}
					if (ele.unitValues) {
						obj['uom_key_height'] =
							(ele?.unitValues?.coords[3] - ele?.unitValues?.coords[1]) / 1123;
						...
						obj['uom_key_width'] =
							ele?.unitValues?.coords[2] - ele?.unitValues?.coords[0];
						obj['uom_page'] = 1;
						obj['uom_snippet_id'] = ele?.unitValues?.snippetID;
						obj['uom_value_height'] =
							(ele?.unitValues?.valueCoords[3] -
								ele?.unitValues?.valueCoords[1]) /
							1123;
						...
						obj['uom_value_width'] =
							(ele?.unitValues?.valueCoords[2] -
								ele?.unitValues?.valueCoords[0]) /
							868;
					}
					if (ele.timeValues) {
						obj['time_key_height'] =
							(ele?.timeValues?.coords[3] - ele?.timeValues?.coords[1]) / 1123;
						...
						obj['time_key_width'] =
							ele?.timeValues?.coords[2] - ele?.timeValues?.coords[0];
						obj['time_page'] = 1;
						obj['time_snippet_id'] = ele?.timeValues?.snippetID;
						obj['time_value_height'] =
							(ele?.timeValues?.valueCoords[3] -
								ele?.timeValues?.valueCoords[1]) /
							1123;
						...
						obj['time_value_width'] =
							(ele?.timeValues?.valueCoords[2] -
								ele?.timeValues?.valueCoords[0]) /
							868;
					}
					if (ele.dateValues) {
						obj['date_key_height'] =
							(ele?.dateValues?.coords[3] - ele?.dateValues?.coords[1]) / 1123;
						...
						obj['date_key_width'] =
							ele?.dateValues?.coords[2] - ele?.dateValues?.coords[0];
						obj['date_page'] = 1;
						obj['date_snippet_id'] = ele?.dateValues?.snippetID;
						obj['date_value_height'] =
							(ele?.dateValues?.valueCoords[3] -
								ele?.dateValues?.valueCoords[1]) /
							1123;
						...
						obj['date_value_width'] =
							(ele?.dateValues?.valueCoords[2] -
								ele?.dateValues?.valueCoords[0]) /
							868;
					}
					arr.push(obj);
				});
	...
	const openNotification = val => {
		...
		let disc =
			formValues[activeKey]?.name == '' ||
			formValues[activeKey]?.name == undefined
				? 'Please Enter Name '
				: 'Please Enter Method';
		notification.open({
			message: 'Error',
	...
	const findTemplate = async () => {
		...
		if (formValues[activeKey]?.values) {
			(obj['color'] = 'blue'),
				(obj['param_key_height'] =
					(formValues[activeKey]?.values?.anchorCoords[3] -
						formValues[activeKey]?.values?.anchorCoords[1]) /
					imageHeight);
			obj['param_key_left'] =
				formValues[activeKey]?.values?.anchorCoords[0] / imageWidth;
			obj['param_key_text'] = formValues[activeKey]?.values?.anchorValue;
			obj['param_key_top'] =
				formValues[activeKey]?.values?.anchorCoords[1] / imageHeight;
			obj['param_key_width'] =
				formValues[activeKey]?.values?.anchorCoords[2] -
				formValues[activeKey]?.values?.anchorCoords[0];
			obj['param_page'] = 1;
			obj['param_snippet_id'] = formValues[activeKey]?.values?.snippetID;
			obj['param_value_height'] =
				(formValues[activeKey]?.values?.valueCoords[3] -
					formValues[activeKey]?.values?.valueCoords[1]) /
				imageHeight;
			obj['param_value_left'] =
				formValues[activeKey]?.values?.valueCoords[0] / imageWidth;
			obj['param_value_text'] = formValues[activeKey]?.values?.anchorId;
			obj['param_value_top'] =
				formValues[activeKey]?.values?.valueCoords[1] / imageHeight;
			obj['param_value_width'] =
				(formValues[activeKey]?.values?.valueCoords[2] -
					formValues[activeKey]?.values?.valueCoords[0]) /
				imageWidth;
		}
		if (formValues[activeKey]?.unitValues) {
			obj['uom_key_height'] =
				(formValues[activeKey]?.unitValues?.coords[3] -
					formValues[activeKey]?.unitValues?.coords[1]) /
				imageHeight;
			obj['uom_key_left'] =
				formValues[activeKey]?.unitValues?.coords[0] / imageWidth;
			obj['uom_key_text'] = formValues[activeKey]?.unitValues?.unitAnchor;
			obj['uom_key_top'] =
				formValues[activeKey]?.unitValues?.coords[1] / imageHeight;
			obj['uom_key_width'] =
				formValues[activeKey]?.unitValues?.coords[2] -
				formValues[activeKey]?.unitValues?.coords[0];
			obj['uom_page'] = 1;
			obj['uom_snippet_id'] = formValues[activeKey]?.unitValues?.snippetID;
			obj['uom_value_height'] =
				(formValues[activeKey]?.unitValues?.valueCoords[3] -
					formValues[activeKey]?.unitValues?.valueCoords[1]) /
				imageHeight;
			obj['uom_value_left'] =
				formValues[activeKey]?.unitValues?.valueCoords[0] / imageWidth;
			obj['uom_value_text'] = formValues[activeKey]?.unitValues?.unitId;
			obj['uom_value_top'] =
				formValues[activeKey]?.unitValues?.valueCoords[1] / imageHeight;
			obj['uom_value_width'] =
				(formValues[activeKey]?.unitValues?.valueCoords[2] -
					formValues[activeKey]?.unitValues?.valueCoords[0]) /
				imageWidth;
		}
		if (formValues[activeKey]?.timeValues) {
			obj['time_key_height'] =
				(formValues[activeKey]?.timeValues?.coords[3] -
					formValues[activeKey]?.timeValues?.coords[1]) /
				imageHeight;
			obj['time_key_left'] =
				formValues[activeKey]?.timeValues?.coords[0] / imageWidth;
			obj['time_key_text'] = formValues[activeKey]?.timeValues?.timeAnchor;
			obj['time_key_top'] =
				formValues[activeKey]?.timeValues?.coords[1] / imageHeight;
			obj['time_key_width'] =
				formValues[activeKey]?.timeValues?.coords[2] -
				formValues[activeKey]?.timeValues?.coords[0];
			obj['time_page'] = 1;
			obj['time_snippet_id'] = formValues[activeKey]?.timeValues?.snippetID;
			obj['time_value_height'] =
				(formValues[activeKey]?.timeValues?.valueCoords[3] -
					formValues[activeKey]?.timeValues?.valueCoords[1]) /
				imageHeight;
			obj['time_value_left'] =
				formValues[activeKey]?.timeValues?.valueCoords[0] / imageWidth;
			obj['time_value_text'] = formValues[activeKey]?.timeValues?.timeId;
			obj['time_value_top'] =
				formValues[activeKey]?.timeValues?.valueCoords[1] / imageHeight;
			obj['time_value_width'] =
				(formValues[activeKey]?.timeValues?.valueCoords[2] -
					formValues[activeKey]?.timeValues?.valueCoords[0]) /
				imageWidth;
		}
		if (formValues[activeKey]?.dateValues) {
			obj['date_key_height'] =
				(formValues[activeKey]?.dateValues?.coords[3] -
					formValues[activeKey]?.dateValues?.coords[1]) /
				imageHeight;
			obj['date_key_left'] =
				formValues[activeKey]?.dateValues?.coords[0] / imageWidth;
			obj['date_key_text'] = formValues[activeKey]?.dateValues?.dateAnchor;
			obj['date_key_top'] =
				formValues[activeKey]?.dateValues?.coords[1] / imageHeight;
			obj['date_key_width'] =
				formValues[activeKey]?.dateValues?.coords[2] -
				formValues[activeKey]?.dateValues?.coords[0];
			obj['date_page'] = 1;
			obj['date_snippet_id'] = formValues[activeKey]?.dateValues?.snippetID;
			obj['date_value_height'] =
				(formValues[activeKey]?.dateValues?.valueCoords[3] -
					formValues[activeKey]?.dateValues?.valueCoords[1]) /
				imageHeight;
			obj['date_value_left'] =
				formValues[activeKey]?.dateValues?.valueCoords[0] / imageWidth;
			obj['date_value_text'] = formValues[activeKey]?.dateValues?.dateId;
			obj['date_value_top'] =
				formValues[activeKey]?.dateValues?.valueCoords[1] / imageHeight;
			obj['date_value_width'] =
				(formValues[activeKey]?.dateValues?.valueCoords[2] -
					formValues[activeKey]?.dateValues?.valueCoords[0]) /
				868;
		}
	...
	const showModal = async () => {
		...
		if (formValues[activeKey]?.values) {
			(obj['color'] = 'blue'),
				(obj['param_key_height'] =
					...
					1123);
			obj['param_key_left'] =
				formValues[activeKey]?.values?.anchorCoords[0] / 868;
			obj['param_key_text'] = formValues[activeKey]?.values?.anchorValue;
			obj['param_key_top'] =
				formValues[activeKey]?.values?.anchorCoords[1] / 1123;
			obj['param_key_width'] =
				(formValues[activeKey]?.values?.anchorCoords[2] -
					formValues[activeKey]?.values?.anchorCoords[0]) /
				868;
			...
			obj['param_value_height'] =
				...
				1123;
			obj['param_value_left'] =
				formValues[activeKey]?.values?.valueCoords[0] / 868;
			obj['param_value_text'] = formValues[activeKey]?.values?.anchorId;
			obj['param_value_top'] =
				formValues[activeKey]?.values?.valueCoords[1] / 1123;
			obj['param_value_width'] =
				(formValues[activeKey]?.values?.valueCoords[2] -
					formValues[activeKey]?.values?.valueCoords[0]) /
				868;
		...
		if (formValues[activeKey]?.unitValues) {
			...
			obj['uom_value_height'] =
				...
				1123;
			obj['uom_value_left'] =
				formValues[activeKey]?.unitValues?.valueCoords[0] / 868;
			obj['uom_value_text'] = formValues[activeKey]?.unitValues?.unitId;
			obj['uom_value_top'] =
				formValues[activeKey]?.unitValues?.valueCoords[1] / 1123;
			obj['uom_value_width'] =
				(formValues[activeKey]?.unitValues?.valueCoords[2] -
					formValues[activeKey]?.unitValues?.valueCoords[0]) /
				868;
		...
		if (formValues[activeKey]?.timeValues) {
			...
			obj['time_value_height'] =
				...
				1123;
			obj['time_value_left'] =
				formValues[activeKey]?.timeValues?.valueCoords[0] / 868;
			obj['time_value_text'] = formValues[activeKey]?.timeValues?.timeId;
			obj['time_value_top'] =
				formValues[activeKey]?.timeValues?.valueCoords[1] / 1123;
			obj['time_value_width'] =
				(formValues[activeKey]?.timeValues?.valueCoords[2] -
					formValues[activeKey]?.timeValues?.valueCoords[0]) /
				868;
		...
		if (formValues[activeKey]?.dateValues) {
			...
			obj['date_value_height'] =
				...
				1123;
			obj['date_value_left'] =
				formValues[activeKey]?.dateValues?.valueCoords[0] / 868;
			obj['date_value_text'] = formValues[activeKey]?.dateValues?.dateId;
			obj['date_value_top'] =
				formValues[activeKey]?.dateValues?.valueCoords[1] / 1123;
			obj['date_value_width'] =
				(formValues[activeKey]?.dateValues?.valueCoords[2] -
					formValues[activeKey]?.dateValues?.valueCoords[0]) /
				868;
			...
			<div className='custom-wrapper pbr-wrapper'>
				<div className='sub-header'>
					<div className='sub-header-title'>
						...
						<span className='header-title'>Paper Batch Records /</span>
			...
			<div className='pbrTemplateRowContainer'>
				<div className='pbrTemplateLeft'>
					<div className='pbrPanel pbrRightPanel'>
						<Sider trigger={null} collapsible collapsed={leftPanelCollapsed}>
							<span className='trigger' onClick={toggleLeftCollapsed}>
							...
							<Collapse
								...
								<Panel header='Add Parameter' key='2'>
									<Form
										...
										initialValue={{
											dymamic_sections: [
												{ name: 'asdf', method: 'absolute_coordinate' },
												{ name: 'zxcv', method: 'absolute_coordinate' },
											],
										}}
										className='formNewTemplate'
										onFinish={onFinish}
															...
															<>
																<Collapse
																	...
																	{fields.map(({ key, name, ...restField }) => (
																		<Panel
																			header={`Parameter ${key + 1} created`}
																			key={`${key}`}>
																			<div className='addParameterBlock'>
																				{/* {paramaterAdded ? ( */}
																				<div className='parameterAdded-block'>
																					...
																					<div className='parameterAddingBlock parameterValueBlock'>
																						...
																						<p>
																							Found in{' '}
																							{`${fileList.length}/${searchedFileList.length}`}{' '}
																							files
																						</p>
																					</div>
																					<div>
					...
					<div className='pbrPanel pbrCenterPanel'>
						<div className='pbrCenterPanel-header'>
							<Row className='pbrCenterPanelRow'>
								<Col span={12} className='pbrCenterPanelCol pbrCenterBlockLeft'>
