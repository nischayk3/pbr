...
const ViewChart = (props, ref) => {
  ...
  const getSiteGlobalFilter = async () => {
    ...
    try {
      ...
      dispatch(hideLoader());
      dispatch(
        showNotification("error", "There is no data for selected filters")
      );
    }
  };
  ...
  const showPreview = async (index) => {
    ...
    let payload = {
      ...
      date_range:
        tempPanels[index].data_filter.date_range == "/"
          ? ""
          : tempPanels[index].data_filter.date_range,
            ...
            el.data[0].data = el.data[0].data.map((item, k) => {
              if (item.mode === "markers") {
                if (!Array.isArray(item.marker.color)) {
                  item.marker.color = Array(item.x.length).fill(
                    item.marker.color
                  );
                }
