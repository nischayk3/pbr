...
import { Button, Dropdown, Empty, Form, Menu } from "antd";
...
const ReportDesignerNew = () => {
  ...
  const esignPublishRes = useSelector(
    (state) => state.commonReducer.publishRes
  );
  ...
  const checkChanges = (reportData, mainJson, save_Type) => {
    ...
    if (layout_change && new_charts_added) return [true, jayson];
    ...
    if (!layout_change && !new_charts_added) return [true, json_data];
  };
  ...
  const PrepareJson = (formData, saveType) => {
    ...
    if (check) {
      ...
      obj["layout_info"] = {
        layout_info: isLoad ? lay_data : formData,
      ...
      if (
        ...
        dispatch(
          showNotification("error", "Report name, view or chart id Is required")
        );
      }
    } else {
    ...
    <div className="custom-wrapper">
      <div className="sub-header" style={{ paddingRight: "24px" }}>
        <div className="sub-header-title">
          ...
          <BreadCrumbWrapper
            urlName={`/dashboard/report_designer/${reportId}`}
            value={reportId}
            data="Untitled"
          />
          {/* </div> */}
