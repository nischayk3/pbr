  ...
  const onClickNode = (node) => {
    if (node.clickType === "backward") {
      ...
      if (node.nodeType === "Material") {
        ...
        getSubProcessNode(_reqSubprocess);
  ...
  const getBackwardGeneology = async (_reqBack) => {
    try {
      ...
      if (backwardRes.statuscode === 200) {
        ...
        setActivateKey("2");
        dispatch(hideLoader());
      } /* istanbul ignore next */ else if (backwardRes.status === 400) {
        /* istanbul ignore next */
        setGenealogyData([]);
        /* istanbul ignore next */
        dispatch(hideLoader());
  ...
  const fileUpload = async (_fileRequest) => {
    try {
      ...
      if (fileResponse.Status === 202) {
        ...
        const filterDuplicateFile = uploadFileDetail.filter(
          (item) => !duplicateFile.includes(item.fileName)
        );
        filterDuplicateFile.map((item) => {
          fileName.push(item.fileName);
          fileSize.push(item.fileSize);
        });
        let timezoneDuplicate = timeZoneList.filter(
          (item) => !duplicateFile.includes(item.fileName)
        );
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  <p className="ant-upload-hint">
                    Upload files of PDF format. You may carry out single or bulk
                    upload. Strictly refrain from uploading company data or
                    other band files
                  </p>
                </Dragger>
              ...
              >
                {fileUploadResponse === 200 && (
                  <Result status="error" title={fileMessage}>
                    {fileMessage && (
                      <div className="desc">
                        {uploadedFileInfo &&
