...
describe('PBR', () => {
    ...
    it('Create and save new template', () => {
        ...
        cy.get('.ant-radio-button-wrapper-checked > :nth-child(2) > .pdfListBlock > span').click({ force: true });
        cy.get(':nth-child(1) > .ant-radio-button > .ant-radio-button-input').check({ force: true });
        cy.get('.ant-modal-footer > .ant-btn > span').click({ force: true });
        ...
        cy.get('.ant-select-item-option-active > .ant-select-item-option-content').click();
        cy.get('#dynamic_form_nest_item_users_0_param_key').click();
        cy.get('[coords="111.94621527194977,57.53743443638084,178.1889796704054,68.96316892467435"]').click({ force: true });
        cy.get('#dynamic_form_nest_item_users_0_param_snippet_value').click();
        cy.get('[coords="205.69649124145508,57.55570698529482,267.6595058441161,68.86165171861641"]').click({ force: true });
        cy.get(':nth-child(3) > :nth-child(5) > .ant-col > .ant-form-item-control-input > .ant-form-item-control-input-content > .ant-select > .ant-select-selector').click();
    ...
    it("Toggle and add parameter", () => {
        ...
        /* ==== Generated with Cypress Studio ==== */
        cy.get('.ant-radio-button-wrapper-checked > .ant-collapse-header').click();
